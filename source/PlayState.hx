...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'elytron':
				dialogue = CoolUtil.coolTextFile(Paths.txt('elytron/song1'));
			case 'granite':
				dialogue = CoolUtil.coolTextFile(Paths.txt('granite/song2'));
			case 'bedrock':
				dialogue = CoolUtil.coolTextFile(Paths.txt('bedrock/song3'));
		}
		...
		switch(SONG.song.toLowerCase())
		{
			...
			case 'elytron':
			{
					defaultCamZoom = 0.85;
					curStage = 'caveDay';
					var bg:FlxSprite = new FlxSprite(-600, -400).loadGraphic(Paths.image('buckBumble/BG_DAY_BACK'));
					bg.antialiasing = true;
					bg.scrollFactor.set(0.9, 0.9);
					bg.active = false;
					add(bg);

					var caveFront:FlxSprite = new FlxSprite(-600, -400).loadGraphic(Paths.image('buckBumble/BG_DAY_FRONT'));
					...
					var stalactites:FlxSprite = new FlxSprite(-600, -300).loadGraphic(Paths.image('buckBumble/STALACTITES_DAY'));
					// stalactites.setGraphicSize(Std.int(stageCurtains.width * 0.9));
					stalactites.updateHitbox();
					stalactites.antialiasing = true;
					stalactites.scrollFactor.set(1.3, 1.3);
					stalactites.active = false;

					add(stalactites);
			}
			case 'granite' | 'bedrock':
			{
					defaultCamZoom = 0.85;
					curStage = 'caveNight';
					var bg:FlxSprite = new FlxSprite(-600, -400).loadGraphic(Paths.image('buckBumble/BG_NIGHT_BACK'));
					bg.antialiasing = true;
					bg.scrollFactor.set(0.9, 0.9);
					bg.active = false;
					add(bg);

					var caveFrontN:FlxSprite = new FlxSprite(-600, -400).loadGraphic(Paths.image('buckBumble/BG_NIGHT_FRONT'));
					caveFrontN.updateHitbox();
					caveFrontN.antialiasing = true;
					caveFrontN.scrollFactor.set(0.9, 0.9);
					caveFrontN.active = false;
					add(caveFrontN);

					var stalactitesN:FlxSprite = new FlxSprite(-600, -300).loadGraphic(Paths.image('buckBumble/STALACTITES_NIGHT'));
					// stalactites.setGraphicSize(Std.int(stageCurtains.width * 0.9));
					stalactitesN.updateHitbox();
					stalactitesN.antialiasing = true;
					stalactitesN.scrollFactor.set(1.3, 1.3);
					stalactitesN.active = false;
		...
		var kadeEngineWatermark = new FlxText(4,healthBarBG.y + 50,0,SONG.song + " " + (storyDifficulty == 2 ? "Hard" : storyDifficulty == 1 ? "Normal" : "Easy"));
		kadeEngineWatermark.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
		kadeEngineWatermark.scrollFactor.set();
		if (FlxG.save.data.accuracyDisplay)
		{
			kadeEngineWatermark.x += 0;
		}
		else
		{
			kadeEngineWatermark.x += 335;
		}
	...
	{
		if (StoryMenuState.reCutscene == 1)
		{
			StoryMenuState.reCutscene = 0;

			if (SONG.song.toLowerCase() == 'elytron')
			{
				camHUD.visible = false;

				var uwu:FlxSprite = new FlxSprite(-150, -100).makeGraphic(FlxG.width * 3, FlxG.height * 2, FlxColor.BLACK);
				uwu.scrollFactor.set();
				add(uwu);
				
				FlxG.switchState(new VideoState('assets/videos/pressftopayyourrespects/bfFallsLikeABitch_lol.webm',new PlayState()));
			}

			if (SONG.song.toLowerCase() == 'bedrock')
			{
				camHUD.visible = false;
				...
				FlxG.switchState(new VideoState('assets/videos/thedudespissedholyfuck/thatFeelingWhen.webm',new PlayState()));
			}
		}
		...
		if (SONG.song.toLowerCase() == 'roses' || SONG.song.toLowerCase() == 'thorns' || SONG.song.toLowerCase() == 'granite')
		{
			remove(black);
		...
		else
		{
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					if (PlayState.SONG.song.toLowerCase()=='bedrock')
					{
